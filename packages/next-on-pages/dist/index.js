...
// ../../node_modules/.pnpm/dedent-tabs@0.10.3/node_modules/dedent-tabs/dist/dedent-tabs.js
var require_dedent_tabs = __commonJS({
  "../../node_modules/.pnpm/dedent-tabs@0.10.3/node_modules/dedent-tabs/dist/dedent-tabs.js"(exports) {
    "use strict";
...
function cliLog(message, fromVercelCli = false) {
  console.log(prepareCliMessage(message, fromVercelCli));
}
function cliError(message, reportIssue = false, fromVercelCli = false) {
...
function cliWarn(message, fromVercelCli = false) {
  console.warn(prepareCliMessage(message, fromVercelCli));
}
function prepareCliMessage(message, fromVercelCli) {
  return (0, import_dedent_tabs.default)(message).split("\n").map((line) => `${getCliPrefix(fromVercelCli)} ${line}`).join("\n");
}
function getCliPrefix(fromVercelCli) {
...
async function getParsedMiddlewareManifest(functionsMap) {
  const middlewareManifest = await readJsonFile(
...
function parseMiddlewareManifest(middlewareManifest, functionsMap) {
...
function matchFunctionEntry(entryName, fileName) {
...
async function getNextJsConfigs() {
...
// src/buildApplication/vercelConfig.ts
var supportedConfigVersion = 3;
async function getVercelConfig() {
  const config = await readJsonFile(".vercel/output/config.json");
  if (!config) {
...
async function buildWorkerFile({ hydratedMiddleware, hydratedFunctions }, vercelConfig, nextJsConfigs, experimentalMinify) {
...
// ../../node_modules/.pnpm/astring@1.8.4_patch_hash=x7ffacyr55kushqehqkmkm7xia/node_modules/astring/dist/astring.mjs
var { stringify } = JSON;
...
async function generateFunctionsMap(functionsDir, experimentalMinify) {
  const processingSetup = {
    functionsDir,
    tmpFunctionsDir: (0, import_path2.join)((0, import_os2.tmpdir)(), Math.random().toString(36).slice(2)),
    tmpWebpackDir: (0, import_path2.join)((0, import_os2.tmpdir)(), Math.random().toString(36).slice(2)),
    experimentalMinify
  };
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (await validateDir(filepath)) {
        const dirResultsPromise = file.endsWith(".func") ? processFuncDirectory(setup, file, filepath) : processDirectoryRecursively(setup, filepath);
...
async function processFuncDirectory(setup, file, filepath) {
  const relativePath = (0, import_path2.relative)(setup.functionsDir, filepath);
  const functionConfig = await readJsonFile(
    (0, import_path2.join)(filepath, ".vc-config.json")
  );
  if (functionConfig?.runtime !== "edge") {
    return {
      invalidFunctions: /* @__PURE__ */ new Set([file])
    };
  }
  const functionFile = (0, import_path2.join)(filepath, functionConfig.entrypoint);
  ...
  const functionsMap = /* @__PURE__ */ new Map();
  const webpackChunks = /* @__PURE__ */ new Map();
  let contents = await (0, import_promises3.readFile)(functionFile, "utf8");
  contents = fixFunctionContents(contents);
  if (setup.experimentalMinify) {
    const { updatedFunctionContents, extractedWebpackChunks } = extractWebpackChunks(setup.tmpWebpackDir, contents, webpackChunks);
    contents = updatedFunctionContents;
    extractedWebpackChunks.forEach(
      (value, key) => webpackChunks.set(key, value)
    );
  }
  const newFilePath = (0, import_path2.join)(setup.tmpFunctionsDir, `${relativePath}.js`);
  await (0, import_promises3.mkdir)((0, import_path2.dirname)(newFilePath), { recursive: true });
  await (0, import_promises3.writeFile)(newFilePath, contents);
  functionsMap.set(
    normalizePath(
      (0, import_path2.relative)(setup.functionsDir, filepath).slice(0, -".func".length)
    ),
...
function extractWebpackChunks(tmpWebpackDir, functionContents, existingWebpackChunks) {
  const webpackChunks = /* @__PURE__ */ new Map();
  ...
  for (const objectOfChunks of expressions) {
    for (const chunkExpression of objectOfChunks) {
      ...
      if (key in existingWebpackChunks) {
        if (existingWebpackChunks.get(key) !== generate(chunkExpression.value)) {
      ...
      const chunkFilePath = (0, import_path2.join)(tmpWebpackDir, `${key}.js`);
      const newValue = {
...
async function buildWebpackChunkFiles(webpackChunks, tmpWebpackDir) {
  for (const [chunkIdentifier, code] of webpackChunks) {
    const chunkFilePath = (0, import_path2.join)(tmpWebpackDir, `${chunkIdentifier}.js`);
    await (0, import_promises3.mkdir)((0, import_path2.dirname)(chunkFilePath), { recursive: true });
    await (0, import_promises3.writeFile)(chunkFilePath, `export default ${code}`);
  }
}
...
async function runVercelBuild() {
  ...
  vercelBuild.stdout.on("data", (data2) => cliLog(`
${data2}`, true));
  vercelBuild.stderr.on("data", (data2) => cliError(`
${data2}`, false, true));
...
async function buildRoutes() {
  try {
    await (0, import_promises5.writeFile)(
      ...
      { flag: "ax" }
      // don't generate file if it's already manually maintained
    );
  } catch (e) {
    if (e.code != "EEXIST") {
      throw e;
...
async function prepareAndBuildWorker(options) {
  ...
  const functionsDir = (0, import_path3.resolve)(
...
function printInvalidFunctionsErrorMessage(invalidFunctions) {
  cliError(`
...
function runNextOnPages(options) {
